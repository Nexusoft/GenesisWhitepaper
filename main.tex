...
\paragraph{\textbf{\textit{Genesis} White Paper} - \textit{1.0.3} - Last Revised \today  }
...
\noindent In 1994, Peter Shor created a method which could be used to solve the Integer Factorization Problem \textit{IFP} and the Discrete Logarithm Problem \textit{DLP} in polynomial time on a quantum computer \cite{shorarxiv}.
...
This is a more difficult problem to solve classically than conventional cyrptography relying on the \textit{DLP} alone.\cite{prooszalka} % Section 2.1.2
...
To mitigate against quantum attack at this time, signature chains are being developed as a part of the Tritium protocol taking advantage of \textit{PKH} and \textit{OTS} or one-time signatures, which preventing side channel attacks to make quantum attacks exlusive to Grover's Algorithm.
...
There are many promising solutions to the post-quantum conundrum that we are facing in the coming 5 years. 
...
Nexus exists as a publicly available, free-to-use, peer-to-peer network for people to connect and improve the efficiency and quality of their lives.  
...
It is through this connection that people will create a future for themselves filled with life, liberty and the pursuit of happiness.
Welcome to Nexus.
\pagebreak
...
If we neglect some of these ideas, the formulation of the rest of the content will suffer lack of understanding and therefore reduce the capacity of the ideals of Nexus; which is simply an idea of connection between areas of knowledge, people, computers, and business.
