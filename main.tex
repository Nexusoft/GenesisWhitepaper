...
\paragraph{\textbf{White Paper} - \textit{1.0.0} }
...
Neuxs uses a newer, more secure hashing algorithm than that of Bitcoin.
...
Where $T_m$ is the time in minutes and $V_{d}$, $V_{a}$, $V_{m}$ are the reserve deposit amounts for miner, ambassador, and developer reserves.
...
\resizebox{.75\hsize}{!}{$\left\{
\begin{array}{ll}
      V_{b} = \frac{T_{a}}{60} \cdot V_{r} & T_{m} \leq 2.5 Minutes \\
      V_{b} = 2.5 \cdot V_{r} & T_{m} > 2.5 Minutes \\
      V_{b} = 3.0 \cdot V_{r} & T_{m} \geq 20 Minutes \\
\end{array} \right\} $}
\end{equation}
...
As one receives blocks by using this method in a timely manner, one's trust grows.
The more trust \ref{subsection:Trust} gained in the network,
the higher the minting rate earned.\\ 

\noindent The minting rate $R_m$ ranges from 0.5 \% - 3 \%.
...
Dense prime clusters are prime k-tuples with the characteristic that many prime numbers are found in a small interval \cite{wolframprimecluster,primesolominer}.
Finding these dense prime clusters is a trial-and-error process.
Miners must try many numbers to successfully find one that is the beginning of a dense prime cluster.\\ 
...
This means the network incentivizes users to contribute to the security of the system. 
...
The minting rate was determined to be a varibale rate between .5\% and 3\% to provide a small natural growth to the coin supply allowing for increased economic opportunity.
...
\resizebox{.75\hsize}{!}{$\left\{
\begin{array}{ll}
      Mod_{d} = [1 - (Min_{d} \cdot T_{o})] \cdot T_{t} & T_{a} \geq T_{t} \\
      Mod_{d} = [1 + (Max_{d} \cdot T_{o})] \cdot T_{t} & T_{a} \leq T_{t} \\
\end{array} 
\right\}$}
\end{equation}\\ 

\noindent Where $T_t$ is the target time, $Min_d$ is the minimum difficulty decrease and $Max_d$ is the maximum difficulty increase.
...
Nodes which have a vested interest in and are acting to benefit the whole network by participating honestly and consistently are considered more trustworthy.\footnote{Not everybody has money, but everybody has time; this is how trust is created.}
...
\begin{equation}
\resizebox{.95\hsize}{!}{$\left\{
\begin{array}{ll}
      W_{t} = \frac{16.5 \cdot ln(\frac{2 \cdot a_t}{2419200}) + 1}{ln(3)} + 1.0 & W_{t} \leq 17.5\\
      W_{b} = \frac{19.0 \cdot ln(\frac{2 \cdot a_b}{86400}) + 1}{ln(3)} + 0.05 & W_{t} \leq 20.0\\
\end{array} 
\right\}$}
\end{equation}
...
In order to prove one's contribution time, a system has been developed to allow an energy-efficient time-based trust system.
...
\begin{equation}
\resizebox{.85\hsize}{!}{$\left\{
\begin{array}{ll}
      W_{g} = \frac{16.5 \cdot ln(\frac{2 \cdot a_{c}}{7257600}) + 1}{ln(3)} + 1.0 & W_{t} \leq 17.5\\
\end{array} 
\right\}$}
\end{equation}
...
These minor modulations have an effect on the value of one second unit time, which can cause inconsistent times on differing machines.
...
This is a more difficult problem to solve classically than the \textit{DLP}.\cite{prooszalka} % Section 2.1.2
...
\noindent We now have an opportunity to step away from existing systems by simply changing our focus. We can choose to look away from the systems of the past and focus our energy on the systems of the future. Through the Nexus framework, users now have the tools needed to change the focus and connect through a mathematically and cryptographically validated network. 
Nexus exists as a publicly available, free-to-use, peer-to-peer network for people to connect and improve the efficiency and quality of their lives.  Peer-to-Peer technologies have the capability to help us reshape our world and Nexus represents an important step forward in this process. 
...
It is through this connection that people will have the opportunity to act together in accord with one another, and pursue a future that is filled with wisdom, prosperity and joy. Welcome to Nexus!
...
There are many areas of research currently under development for prospective implementation into the Nexus framework. These include but are not limited to: scalability solutions, contracting engines, the TAO update series (Tritium, Amine, Obsidian), multi-level locking, lower level database enhancements, post-quantum cryptography, governance systems, protocol level checks and balances and social organization models.
...
\end{wrapfigure}
%%Preston - Colin fix text wrap in ths section
